...
if (!conn.authState.creds.registered) {
  ...
  if (conn.requestPairingCode) {
    ...
    console.log(chalk.cyan('Tu código de vinculación es:', code))
  }
}
...
async function connectionUpdate (update) {
  ...
  if (connection === 'open') {
    console.log(chalk.yellow('☆ {ℙ𝕚𝕔𝕠𝕝𝕒𝕤𝔸𝕀𝐮𝐥𝐭𝐫𝐚-𝐌𝐃} ☆ conectado correctamente.'))
  }
  ...
  if (reason === 405) {
    ...
    console.log(
      chalk.bold.redBright(
        'Conexión reemplazada. Me voy a reiniciar...\nSi aparece error, ejecuta de nuevo: npm start'
      )
    )
  ...
  if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
      conn.logger.error(
        `Sesión incorrecta, borra la carpeta ${global.authFile} y escanea de nuevo.`
      )
      ...
      conn.logger.error(
        'Conexión reemplazada, se abrió otra sesión. Cierra la otra primero.'
      )
    } else if (reason === DisconnectReason.loggedOut) {
      conn.logger.error(
        `Conexión cerrada, borra la carpeta ${global.authFile} y escanea de nuevo.`
      )
    } else if (reason === DisconnectReason.restartRequired) {
      conn.logger.info('Reinicio requerido, reinicia el servidor si hay problemas.')
      await global.reloadHandler(true).catch(console.error)
