...
/**
 * Convierte audio a formato Opus PTT compatible con WhatsApp
 * @param {Buffer} audioBuffer - Buffer del audio original
 * @returns {Promise<Buffer>} Buffer del audio convertido a Opus PTT
 */
const convertToPTTFormat = async (audioBuffer) => {
    const tmpDir = os.tmpdir()
    const timestamp = Date.now()
    const randomId = Math.random().toString(36).substring(7)
    const tmpIn = path.join(tmpDir, `baileys-ptt-in-${timestamp}-${randomId}.tmp`)
    const tmpOut = path.join(tmpDir, `baileys-ptt-out-${timestamp}-${randomId}.ogg`)
    
    try {
        fs.writeFileSync(tmpIn, audioBuffer)
        
        await new Promise((resolve, reject) => {
            const ffmpegCommand = `ffmpeg -y -i "${tmpIn}" -vn -c:a libopus -ar 48000 -ac 1 -b:a 128k -application voip -f ogg "${tmpOut}"`
...
/**
 * Verifica si FFmpeg está disponible y soporta libopus
    ...
    return new Promise((resolve) => {
        exec('ffmpeg -version', (err, stdout) => {
            if (err) {
                ...
                const hasOpus = stdout.toLowerCase().includes('libopus')
                resolve(hasOpus)
            }
        })
    })
...
if (content.audio && content.ptt === true) {
    try {
        ...
        if (!ffmpegAvailable) {
            ...
            if (Buffer.isBuffer(content.audio)) {
                ...
                if (content.audio.startsWith('http://') || content.audio.startsWith('https://')) {
                    ...
                    if (!fs.existsSync(content.audio)) {
                        throw new Error(`File not found: ${content.audio}`)
                    }
                    audioBuffer = fs.readFileSync(content.audio)
                }
            } 
            else if (content.audio && typeof content.audio === 'object') {
                if (content.audio.url !== undefined && content.audio.url !== null) {
                    ...
                    if (Buffer.isBuffer(audioUrl)) {
                        ...
                        throw new Error(`Unsupported format in audio.url: ${typeof audioUrl}`)
                    }
                }
                else if (content.audio.buffer) {
                    if (!Buffer.isBuffer(content.audio.buffer)) {
                        throw new Error('audio.buffer property is not a Buffer')
                    }
                    audioBuffer = content.audio.buffer
                }
                else if (content.audio.path) {
                    if (!fs.existsSync(content.audio.path)) {
                        throw new Error(`File not found: ${content.audio.path}`)
                    }
                    audioBuffer = fs.readFileSync(content.audio.path)
                }
                else {
                    throw new Error('Audio object must have "url", "buffer", or "path" property')
                }
            } 
            else {
                throw new Error(`Unsupported audio format: ${typeof content.audio}`)
            }
            ...
            // Convertir a formato Opus PTT
            const convertedAudio = await convertToPTTFormat(audioBuffer)
            
            if (!convertedAudio || convertedAudio.length === 0) {
                throw new Error('Conversion produced empty audio buffer')
            }
            
            // Aplicar valores correctos para WhatsApp PTT
        ...
        // Limpiar valores problemáticos antes de continuar
        if (content.fileName) {
            delete content.fileName
        }
        content.mimetype = 'audio/ogg; codecs=opus'
